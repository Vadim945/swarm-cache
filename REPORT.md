...
=== ШАГ 3: SELF-SERVE (без ручной раздачи ключей) ===
- Лендинг: GET https://swarm.telscan.ru/ — описание, тарифы, примеры curl, кнопка «Получить ключ».
- POST /register {name} — автоматическая выдача ключа + бонус 10 ед (self-serve, без администратора).
- Защита от накрутки: лимит 3 регистрации/сутки с одного IP (REG_DAILY_LIMIT, по X-Forwarded-For).
- Проверено: новый пользователь beta-user-1 получил ключ, 10 ед, генерация -1 ед, повтор -0.1 ед,
  лимит регистраций сработал на 4-й попытке (429).
- Итог: продукт самодостаточен — пользователь приходит на сайт, сам получает ключ и пробует.

=== ШАГ 4: ДЕМО БЕЗ КЛЮЧА + РЕФЕРАЛКА (виральность без аудитории) ===
- Демо-режим: POST /ask без X-API-Key → до 3 вопросов/сутки с IP (DEMO_DAILY), ответ помечен demo:true,
  отдаётся demo_left. Проверено: кэш 14-50мс, генерация 5.4с, 4-й запрос → 429 «demo limit reached».
- Реферальная программа: REFERRAL_BONUS=2 ед обоим; код REF-XXXXXXXX выдаётся при регистрации (колонка ref_code
  в users, миграция ALTER TABLE для старых БД + бэкфилл). Приём кода: body.ref или ?ref= (share-ссылки).
- Защита: само-рефералка запрещена, 1 начисление на новичка, лимит рефереру 5/сутки (REFERRAL_MAX_PER_DAY).
- Лендинг обновлён: демо-песочница на главной, поле реф-кода, показ share-ссылки после регистрации.
- /pricing: +referral_bonus, demo_free_questions. /balance: +ref_code, share_link.
- Проверено E2E: referee-1 (12 ед), неверный код (10 ед, без бонуса), referee-3 по ?ref= (12 ед),
  реферер user 1: 8.8→10.8, user 2: 8.9→10.9. Коммит 55a6e3e.
- GitHub topics: 12 шт. (llm, semantic-cache, ipfs, p2p, l402, micro-payments, sqlite-vec, deepseek...).
