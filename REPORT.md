...
=== ШАГ 5: ЛЕНДИНГ v2 (понятный обычному человеку) ===
- Вадим: «я бы не понял, что это такое и как пользоваться» → переписал лендинг с нуля:
  - Заголовок о выгоде, а не о технологии («Быстрые и дешёвые ответы ИИ. Повторные вопросы — мгновенно»)
  - Аналогия «умный секретарь» вместо терминов LLM/P2P/L402
  - Блоки: Зачем это нужно (быстро/дёшево/умно), Кому подойдёт (боты, FAQ, сервисы),
    Как начать — 3 шага, Сколько это стоит, Частые вопросы (FAQ)
  - Убраны технические детали (IPFS, эмбеддинги, порог сходства) — только мелкая ссылка на GitHub
- Проверено: страница 200 (15.8KB), демо-вопрос из кэша 24мс, регистрация → ключ+10ед+реф-код.
- Лимиты (3 демо/день, 3 регистрации/день с IP) работают — при тестах с того же IP упирались в 429,
  это защита, а не баг. Сброс счётчиков = restart сервиса (in-memory).
